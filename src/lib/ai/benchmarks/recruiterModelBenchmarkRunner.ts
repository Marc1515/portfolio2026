...
export function formatBenchmarkTerminalSummary(
  ...
  return [
    ...
    "Scoring note: the deterministic score is approximate; review critical failures separately, and keep human review in the model-selection process.",
...
function buildMarkdownReport(report: RecruiterModelBenchmarkReport): string {
  const lines = [
    ...
    `- Warm-up: ${formatSeconds(report.summary.warmupDurationMs)}`,
    "",
    "> The deterministic benchmark score is an approximate aid. Critical failures must be reviewed separately, and human review remains required before comparing models.",
    "",
    "## Cases",
    "",
  ];
  ...
  for (const result of report.cases) {
    lines.push(
      ...
      ...(result.criticalFailureReasons.length > 0
        ? [`- Critical reasons: ${result.criticalFailureReasons.join(", ")}`]
        : []),
...
export async function runRecruiterModelBenchmark(options: {
  ...
  for (const benchmarkCase of selectedCases) {
    ...
    results.push({
      ...
      criticalFailureReasons: scored.criticalFailureReasons,
